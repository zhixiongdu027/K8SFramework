...
func equalTServerAndDaemonSet(tserver *tarsV1beta3.TServer, daemonSet *k8sAppsV1.DaemonSet) bool {
	...
	if !equality.Semantic.DeepEqual(tserver.Spec.K8S.Args, container.Args) {
		return false
	}

	if !equality.Semantic.DeepEqual(tserver.Spec.K8S.Command, container.Command) {
		return false
	}
...
func equalTServerAndStatefulset(tserver *tarsV1beta3.TServer, statefulSet *k8sAppsV1.StatefulSet) bool {
	...
	/*
		...
		If you would like to change statefulSet.Spec.PodManagementPolicy, delete the statefulset and wait for rebuild
	*/
